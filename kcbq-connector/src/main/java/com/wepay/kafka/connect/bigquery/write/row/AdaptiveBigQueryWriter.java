...
public class AdaptiveBigQueryWriter extends BigQueryWriter {
  ...
  public Map<Long, List<BigQueryError>> performWriteRequest(
    ...
    try {
      ...
      if (writeResponse.hasErrors()
        ...
        attemptSchemaUpdate(tableId, topicAndRecordName);
      }
  ...
  private void attemptSchemaUpdate(PartitionedTableId tableId, TopicAndRecordName topicAndRecordName) {
    try {
      schemaManager.updateSchema(tableId.getBaseTableId(), topicAndRecordName);
    } catch (BigQueryException exception) {
      throw new BigQueryConnectException(
