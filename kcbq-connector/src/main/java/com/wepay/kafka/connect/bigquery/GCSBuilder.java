...
public class GCSBuilder {
  ...
  public GCSBuilder(String projectName) {
    ...
    this.keyFile = null;
    setKeyFileType(keyFileType);
  }

  public GCSBuilder setKeyFile(String keyFile) {
  ...
  public Storage build() {
    return connect(projectName, keyFile, keyFileType);
  }
   ...
   * @param keyFile The name of a file/string containing a JSON key that can be used to provide
   *                    credentials to GCS, or null if no authentication should be performed.
   * @param keyFile The config that determines whether the keyFile is a string or filepath
   * @return The resulting Storage object.
   */
  private Storage connect(String projectName, String keyFile, String keyFileType) {
    if (keyFile == null) {
    ...
    if (keyFile != null && keyFileType.equals("JSON")) {
      try (InputStream keyFileStream = new ByteArrayInputStream(keyFile.getBytes())) {
    ...
    try (InputStream credentialsStream = new FileInputStream(keyFile)) {
      logger.debug("Attempting to authenticate with GCS using provided json key file");
      return StorageOptions.newBuilder()
          .setProjectId(projectName)
